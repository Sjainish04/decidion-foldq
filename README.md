...
**What it is.** A reproducible platform that encodes RNA secondary-structure
prediction as a QUBO, solves it with classical, quantum-inspired and gate-based
methods, and attributes every result to the earliest stage that failed.

**What it does not claim.** ViennaRNA already solves pseudoknot-free MFE folding
*exactly* in cubic time. **No quantum-advantage claim is made anywhere in this
repository.** The contribution is a method for being honest about where a
quantum-style formulation does and does not help.

**The three findings that matter:**

| | |
|---|---|
| The formulation is faithful | The reference structure is the QUBO's ground state on **17/19** instances [68.6%, 97.1%], and the surrogate orders candidates *within* a sequence at median Spearman **0.952** |
| QAOA does not beat classical heuristics | **29.6–44.4%** ground-state rate against **100%** for tabu, local search, SQA and simulated annealing — and the sampling budget matters more than circuit depth |
| Disabling one term reaches pseudoknots | Crossing structures dot-bracket notation **cannot express at all**: F1 **1.000** where ViennaRNA scores 0.667 and 0.000 |

**Where the evidence is.** [`results/full/`](results/full) — committed CSV
regenerated by `make reproduce`, with a manifest pinning commit, Python version,
platform and seed. Every number below traces to it. Alongside it:
[figures](results/full/figures), a
[worked failure case](results/full/failure_cases/trna_phe_yeast_acceptor.md),
...
|---|---|---|
| Charge-and-refund beats stacking-only | `e1_formulation.csv`, matched cell by cell | Strong — holds at every penalty setting |
| Stem encoding is smaller at matched representability | `e2_encoding.csv` | Strong — 551 vs 858 variables at 100% Gate A |
| Classical heuristics reach the optimum reliably | `e3_solvers.csv` | Strong — 30/30 determinate runs, four solvers |
| QAOA underperforms them here | `e4_qaoa.csv` | Strong for the claim; **the `reps` trend itself is not separable at n=27** |
| Shot budget matters more than circuit depth | `e4_qaoa.csv` | Moderate — intervals separate, n=27 per cell |
| The formulation reaches pseudoknots | `e5_pseudoknot.csv` | Mechanism demonstrated; **fixtures are constructed, not literature-derived** |
| The surrogate ranks candidates correctly | `e6_surrogate.csv` | Strong — within-sequence, 28 sequences, with intervals |
| Real structures are harder | `e5_pseudoknot.csv` | Strong — tRNA-Phe F1 0.326, reported deliberately |
...
**Implemented and tested (310 Python tests, 171 frontend tests, 21 end-to-end):**
...
- Six experiment runners (E1–E6, the last measuring within-sequence surrogate fidelity) and a `run_all` entry point (`make reproduce`).
...
Also: [In 60 seconds](#in-60-seconds) · [Claims and their evidence](#claims-and-their-evidence)
...
| Reproducible implementation | `uv.lock`, 310 tests, CI on every push, `make reproduce`, run manifests |
